...
export const Crash = () => {
  ...
  const startGame = () => {
    ...
    addTransaction({
      ...
      onSuccess: () => { 
        crashPointRef.current = +(Math.pow(Math.random(), 0.5) * 8.5 + 1.5).toFixed(2);
  ...
  const updateGame = (time: number) => {
    ...
    if (gameStateRef.current === 'CRASHING') {
      ...
      const currentY = START_Y - Math.min(VIEW_HEIGHT, Math.pow(Math.abs(nextMultiplier - 1.0), 0.75) * 35);
    ...
    const currentX = START_X + Math.min(VIEW_WIDTH, elapsed * 55);
    const currentY = START_Y - Math.min(VIEW_HEIGHT, Math.pow(Math.abs(currentMultiplier - 1.0), 0.75) * 35);
  ...
  const cashOut = () => {
    if (gameStateRef.current !== 'RUNNING' && gameStateRef.current !== 'CRASHING') return;
  ...
  useEffect(() => {
    ...
    <div className="flex items-center justify-center p-6 text-foreground min-h-[550px]">
      <div className={`w-full max-w-4xl rounded-xl h-auto border border-border bg-card p-6 shadow-md`}>
        
        <div className="mb-4 space-y-1 text-center">
          <h2 className="text-2xl font-bold tracking-tight text-primary-foreground">КРАШ</h2>
          <p className="text-sm text-muted-foreground">САМАЯ ЧЕСТНАЯ МИНИ-ИГРА</p>
        </div>
        ...
        <div 
          ...
          <svg width={`${VIEW_WIDTH}`} height={`${VIEW_HEIGHT}`} className="absolute inset-0 w-full h-full select-none">
            <defs>
            ...
            <g 
              ...
              className={gameState === 'CRASHING' || gameState === 'CRASHED' ? 'text-destructive' : 'text-foreground'}
            >
          ...
          <div className="absolute top-4 right-6 pointer-events-none select-none text-right">
            {gameState === 'CRASHED' ? (
              ...
              <span className={`text-5xl font-black tabular-nums tracking-tighter transition-all duration-75 ${
                gameState === 'CRASHING' ? 'text-destructive' : 'text-secondary-foreground opacity-90'
              }`}>
        ...
        <div className="space-y-4">
          ...
          {gameState === 'CASHED_OUT' && (
            <div className="rounded-lg bg-primary/20 border border-primary p-4 text-center text-primary-foreground animate-fade-in">
              <p className="text-sm font-medium">НЕВОЗМОЖНО... ВЫ ВЫЙГРАЛИ!!!</p>
              <p className="text-3xl font-extrabold mt-1">+{winAmount} ₽</p>
              <p className="text-xs text-muted-foreground mt-1">Коэффициент ставки: {cashedOutMultiplier}x</p>
            </div>
          )}

          {gameState === 'IDLE' && (
            <div className="space-y-2">
              <label className="text-sm font-medium text-muted-foreground">Ваша ставка (₽)</label>
              <input
                type="number"
                disabled={false}
                value={betInput}
